...
(function ($) {
    ...
    AjaxLoadMore.init = function () {
        ...
        if ($path === undefined) {
            $path = './wp-content/themes/dan-pearlman';
        }
        ...
        var type_post  = $content.attr('data-post-type'); 
        
        $('#load-more').text("Loading...");
        // Load posts function
        AjaxLoadMore.loadPosts = function ( numpost) {
            ...
            setTimeout(function() {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if ($data.length > 0) {
                        	//var $el = $('section.list');
                            var $el = $('<div class="' + $prefix + 'content"></div>');
                        	$el.append($data);
                        	$el.hide();
                        	$content.append($el);
        ...
        $button.click(function () {
            if (!$loading && !$finished && !$(this).hasClass('done')) {
                ...
                page++;
                if( type_post == 'jobs')
					AjaxLoadMore.loadPosts( 6 );
				else
					AjaxLoadMore.loadPosts( 7 );
            }
        });
        ...
        if ($scroll) {
            $window.scroll(function () {
                ...
                if (!$loading && !$finished && $window.scrollTop() >= Math.round(content_offset.top - ($window.height() - 0)) &&  proceed) {
                    $loading = true;
                    page++;
                    if( type_post == 'jobs')
						AjaxLoadMore.loadPosts( 6 );
